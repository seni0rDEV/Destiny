...
// JavaScript to close the hamburger menu when a link is clicked
document.addEventListener('DOMContentLoaded', (event)=>{
  function toggleMenu() {
    document.querySelector('.nav').classList.toggle('active')
    document.querySelector('.nav-toggler').classList.toggleMenu('active')
  }
  document.querySelector('.nav-toggler').addEventListener('click', toggleMenu);
  document.querySelectorAll('.nava').forEach(navvLink => {
